...
@author: adapted from https://github.com/RokIvansek/
"""
...
class spectral():
    """X is a real matrix nxm, labels is a vector of integers in range 0:(n-1) s is a similaritiy function"""
...
#        self.latex = []
    ...
    def kNN_graph(self, k, metric, mutual=True):
        nn = NearestNeighbors(k, algorithm="auto").fit(self.X)
        print("nn", nn)
...
#                self.w, self.U = eigsh(self.L, k) #compute the eigenvalues and eigenvectors using method for sparse matrices
            kmeans = KMeans(k)
            kmeans_pred = kmeans.fit_predict(self.U) #use the kmeans to calculate centroids of voronoi
            self.pred = kmeans_pred
        self.clustering = "unnormalized"
...
#                self.w, self.U = eigsh(self.L, k, self.D) #using method for sparse matrices
            kmeans = KMeans(k)
            kmeans_pred = kmeans.fit_predict(self.U)
            self.pred = kmeans_pred
        self.clustering = "normalized_rw"
        return kmeans, kmeans_pred
...
#                self.w, self.U = eigsh(self.L, k, self.D) #using method for sparse matrices
            ...
            kmeans = KMeans(k)
            kmeans_pred = kmeans.fit_predict(self.T)
            self.pred = kmeans_pred
        self.clustering = "normalized_sym"
        return kmeans, kmeans_pred
    ...
    def evaluate(self):
        ...
        ari = str("Adjusted Rand index:"+"%.4f" % ARI)
        ami = str("Adjusted Mutual Information:"+"%.4f" % AMI)
        nmi = str("Normalized Mutual Information:"+"%.4f" % NMI)
        return ari, ami, nmi
